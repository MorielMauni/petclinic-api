    ...
    stages {
        ...
        stage('Get API Testing Repo') {
            steps {
                ...
                git(
                    url: 'git@github.com:MorielMauni/petclinic-api.git',
                    credentialsId: 'api',
                    branch: "main"
                )
            }
            ...
            steps {
                ...
                dir('./api-test') {
                    ...
                    script {
                        ...
                        sh 'ls -l'
                        sh '/opt/jmeter/bin/jmeter -n -t ./test-plan.jmx -l ./test-results.csv'  // Fixed path
                        sh 'mkdir html-report'
                        sh 'ls -l'
                        echo "create html report"
                        sh '/opt/jmeter/bin/jmeter -g ./test-results.csv -e -o html-report'  // Fixed path
